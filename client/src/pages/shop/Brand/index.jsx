...
import { useGetBrandByIdQuery } from '../../../redux/apis/subcategoriesApi.js'
...
// ============================HookCategory====================
const useFetchProductsByCategoryId = (
  categoryItemId,
  subItemId,
  categoryId,
  subId,
  ...
  useEffect(() => {
    if (categoryId && categoryItemId && subItemId) {
      params.set('category', categoryItemId)
      params.set('subCategory', subItemId)
      params.set('subSubCategory', categoryId)
      params.set('limit', 24)
      params.set('page', pageNumber)
      getAllProducts(
        // `brand=${categoryId}&limit=1000${query ? `&${query}` : ''}`
  ...
  }, [query, categoryId, categoryItemId, subItemId, pageNumber])
  return {
...
// ============================   HookCategory      ====================
export const SubCategoryBrandProductsPage = (props) => {
  const { brandId: categoryId, categoryItemId, subItemId } = useParams()
  const { data: brandItem } = useGetBrandByIdQuery(categoryId)
  ...
  let findedBrand =
    findedSub &&
    findedSub?.subSubCategory?.find((subSub) => subSub._id === categoryId)
  return (
    <>
      {/* Get Category Meta */}
